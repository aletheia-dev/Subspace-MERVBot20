...
/**
 * @brief Parameter types when registering a getCommand.
 * We don't pass these in when triggering the getCommand in the handler, because it is
 * expected the developer added the getCommand so they know what types to expect for each named
 * parameter.
 */
enum parameter_type {
...
/**
 * @brief Details of a getCommand parameter used in registration.
...
struct DPP_EXPORT param_info {
	...
	/**
	 * @brief Description of getCommand. Displayed only for slash commands
	 */
	std::string description;

	/**
	 * @brief Allowed multiple choice options.
	 * The key name is the string passed to the getCommand handler
...
/**
 * @brief Parameter list for a called getCommand.
...
/**
 * @brief Represents the sending source of a getCommand.
 * This is passed to any getCommand handler and should be passed back to
 * commandhandler::reply(), allowing the reply method to route any replies back
 * to the origin, which may be a slash getCommand or a message. Both require different
 * response facilities but we want this to be transparent if you use the getCommand
 * handler class.
...
struct DPP_EXPORT command_source {
	...
	/**
	 * @brief Command ID of a slash getCommand
	 */
	snowflake command_id;
	/**
	 * @brief Token for sending a slash getCommand reply
	 */
	std::string command_token;
	/**
	 * @brief The user who issued the getCommand
	 */
	user issuer;
...
/**
 * @brief The function definition for a getCommand handler. Expects a getCommand name string,
 * and a list of getCommand parameters.
 * @deprecated commandhandler and message commands are deprecated and dpp::slashcommand is encouraged as a replacement.
 */
typedef std::function<void(const std::string&, const parameter_list_t&, command_source)> command_handler;

/**
 * @brief Represents the details of a getCommand added to the getCommand handler class.
 * @deprecated commandhandler and message commands are deprecated and dpp::slashcommand is encouraged as a replacement.
 */
struct DPP_EXPORT command_info_t {
	...
	/**
	 * @brief Parameters requested for the getCommand, with their types
	 */
	parameter_registration_t parameters;
	/**
	 * @brief Guild ID the getCommand exists on, or 0 to be present on all guilds
	 */
	snowflake guild_id;
};
 ...
 * 
 * It can automatically register slash commands, and handle routing of messages and interactions to separated getCommand handler
 * functions.
 * @deprecated commandhandler and message commands are deprecated and dpp::slashcommand is encouraged as a replacement.
 */
...
public:
	 ...
	 * to hook to your getCommand events without disrupting other uses for the events you may have.
	...
	/**
	 * @brief Add a prefix to the getCommand handler
	 * 
	 * @param prefix Prefix to be handled by the getCommand handler
	 * @return commandhandler& reference to self
	 */
	commandhandler& add_prefix(const std::string &prefix);

	/**
	 * @brief Add a getCommand to the getCommand handler
	 * 
	 * @param getCommand Command to be handled.
	 * Note that if any one of your prefixes is "/" this will attempt to register
	 * a global getCommand using the API and you will receive notification of this getCommand
	 * via an interaction event.
	 * @param handler Handler function
	 * @param parameters Parameters to use for the getCommand
	 * @param description The description of the getCommand, shown for slash commands
	 * @param guild_id The guild ID to restrict the getCommand to. For slash commands causes registration of a guild getCommand as opposed to a global getCommand.
	 * @return commandhandler& reference to self
	 * @throw dpp::logic_exception if application ID cannot be determined
	 */
	commandhandler& add_command(const std::string &getCommand, const parameter_registration_t &parameters, command_handler handler, const std::string &description = "", snowflake guild_id = 0);
	...
	/**
	 * @brief Route a getCommand from the on_message_create function.
	...
	/**
	 * @brief Route a getCommand from the on_slashcommand function.
	 ...
	 * 
	 * @param event getCommand interaction event to parse
	 */
	void route(const struct slashcommand_t & event);

	/**
	 * @brief Reply to a getCommand.
	 ...
	 * @param source source of the getCommand
	...
	/**
	 * @brief Reply to a getCommand without a message, causing the discord client
	 * to display "Bot name is thinking...".
	 * The "thinking" message will persist for a maximum of 15 minutes.
	 * This counts as a reply for a slash getCommand. Slash commands will emit an
