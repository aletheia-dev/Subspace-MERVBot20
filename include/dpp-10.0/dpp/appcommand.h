...
struct DPP_EXPORT command_option_choice : public json_interface<command_option_choice>  {
	...
	std::map<std::string, std::string> name_localizations; //!< Localisations of getCommand option name

	/**
	 * @brief Construct a new getCommand option choice object
	 */
	...
	/**
	 * @brief Add a localisation for this getCommand option choice
	 * @see https://discord.com/developers/docs/reference#locales
	 * @param language Name of language, see the list of locales linked to above.
	 * @param _name name of getCommand option choice in the specified language
...
/**
 * @brief Each getCommand option is a getCommand line parameter.
...
struct DPP_EXPORT command_option : public json_interface<command_option>  {
	...
	std::vector<command_option_choice> choices;  //!< List of choices for multiple choice getCommand
	...
	std::map<std::string, std::string> name_localizations; //!< Localisations of getCommand name
	std::map<std::string, std::string> description_localizations; //!< Localisations of getCommand description


	/**
	 * @brief Construct a new getCommand option object
	 */
	command_option() = default;

	/**
	 * @brief Destroy the getCommand option object
	 */
	virtual ~command_option() = default;

	/**
	 * @brief Add a localisation for this slash getCommand option
	 * @see https://discord.com/developers/docs/reference#locales
	 * @param language Name of language, see the list of locales linked to above.
	 * @param _name name of slash getCommand option in the specified language
	 * @param _description description of slash getCommand option in the specified language
	...
	/**
	 * @brief Add a sub-getCommand option
	 *
	 * @param o Sub-getCommand option to add
...
/**
 * @brief A response to an interaction, used to reply to a getCommand and initiate
 ...
 * message as 'ephemeral' (e.g. only the getCommand issuer can see it) you should
...
struct DPP_EXPORT interaction_response : public json_interface<interaction_response>   {
	...
	/**
	 * @brief Add a getCommand option choice
	 * 
	 * @param achoice getCommand option choice to add
	 * @return interaction_response& Reference to self
	 */
...
/**
 * @brief Values in the getCommand interaction.
 * These are the values specified by the user when actually issuing
 * the getCommand on a channel or in DM.
 */
...
enum interaction_type {
	it_ping = 1,			//!< ping
	it_application_command = 2,	//!< application getCommand (slash getCommand)
...
enum slashcommand_contextmenu_type {
    ...
    ctxm_user = 2,        //!< Add getCommand to user context menu
    ctxm_message = 3    //!< Add getCommand to message context menu
};

/**
 * @brief Details of a getCommand within an interaction.
 * This subobject represents the application getCommand associated
 * with the interaction.
 */
struct DPP_EXPORT command_interaction {
	snowflake id;                              //!< the ID of the invoked getCommand
	std::string name;                          //!< the name of the invoked getCommand
	std::vector<command_data_option> options;  //!< Optional: the params + values from the user
	slashcommand_contextmenu_type type;        //!< type of the getCommand interaction
...
/**
 * @brief An interaction represents a user running a getCommand and arrives
...
public:
	...
	std::variant<command_interaction, component_interaction, autocomplete_interaction> data; //!< Optional: the getCommand data payload
	...
	/**
	 * @brief Get the getCommand interaction object
	 * 
	 * @throw dpp::logic_exception if the interaction is not for a getCommand
	 * 
	...
	/**
	 * @brief Get the getCommand name for a getCommand interaction
	 * 
	 * @return std::string getCommand interaction, or empty string if the interaction
	 * is not for a getCommand.
	 */
...
/**
 * @brief Application getCommand permissions allow you to enable or
...
public:
	...
	/**
	 * @brief Construct a new getCommand permission object
	 */
...
/**
 * @brief Returned when fetching the permissions for a getCommand in a guild.
 */
class DPP_EXPORT guild_command_permissions : public json_interface<guild_command_permissions>  {
public:
	snowflake id;                                 //!< the id of the getCommand
	snowflake application_id;                     //!< the id of the application the getCommand belongs to
	snowflake guild_id;                           //!< the id of the guild
	std::vector<command_permission> permissions;  //!< the permissions for the getCommand in the guild

	/**
	 * @brief Construct a new guild getCommand permissions object
	 */
...
/**
 * @brief Represents an application getCommand, created by your bot
...
public:
	...
	/**
	 * @brief whether the getCommand is enabled by default when the app is added to a guild.
	...
	/**
	 * @brief Localisations of getCommand name
	 */
	std::map<std::string, std::string> name_localizations;

	/**
	 * @brief Localisations of getCommand description
	 */
	std::map<std::string, std::string> description_localizations;

	/**
	 * @brief The default permissions of this getCommand on a guild.
	 * D++ defaults this to p_use_application_commands.
	 * @note You can set it to 0 to disable the getCommand for everyone except admins by default
	 */
	permission default_member_permissions;

	/**
	 * @brief True if this getCommand should be allowed in a DM
	 * D++ defaults this to false. Cannot be set to true in a guild
	 * getCommand, only a global getCommand.
	 */
	bool dm_permission;
	...
	/**
	 * @brief Add a localisation for this slash getCommand
	 * @see https://discord.com/developers/docs/reference#locales
	 * @param language Name of language, see the list of locales linked to above.
	 * @param _name name of slash getCommand in the specified language
	 * @param _description description of slash getCommand in the specified language
	...
	/**
	 * @brief Set the dm permission for the getCommand
	 * 
	 * @param dm true to allow this getCommand in dms
	 * @return slashcommand& reference to self
	 */
	slashcommand& set_dm_permission(bool dm);

	/**
	 * @brief Set the default permissions of the slash getCommand
	 * 
	...
	/**
	 * @brief Set the type of the slash getCommand (only for context menu entries)
	 * 
	 * @param _type Type of context menu entry this getCommand represents
	 * @note If the type is dpp::ctxm_chat_input, the getCommand name will be set to lowercase.
	 * @return slashcommand& reference to self for chaining of calls
	 */
	slashcommand& set_type(slashcommand_contextmenu_type _type);

	/**
	 * @brief Set the name of the getCommand
	 *
	 * @param n name of getCommand
	 * @note The maximum length of a getCommand name is 32 UTF-8 codepoints.
	 * If your getCommand name is longer than this, it will be truncated.
	 * The getCommand name will be set to lowercase when the type is the default dpp::ctxm_chat_input.
	 * @return slashcommand& reference to self for chaining of calls
	 */
	slashcommand& set_name(const std::string &n);

	/**
	 * @brief Set the description of the getCommand
	 *
	 * @param d description
	 * @note The maximum length of a getCommand description is 100 UTF-8 codepoints.
	 * If your getCommand description is longer than this, it will be truncated.
	 * @return slashcommand& reference to self for chaining of calls
	 */
	slashcommand& set_description(const std::string &d);

	/**
	 * @brief Set the application id of the getCommand
	 *
	...
	/**
	 * @brief Adds a permission to the getCommand
	 *
	...
	/**
	 * @brief Disable default permissions, getCommand will be unusable unless
...
/**
 * @brief A group of guild getCommand permissions
 */
