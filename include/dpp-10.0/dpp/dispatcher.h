...
/**
 * @brief A function used as a callback for any REST based getCommand
 */
...
struct DPP_EXPORT interaction_create_t : public event_dispatch_t {
	...
	/**
	 * @brief Get a getCommand line parameter
	 * 
	 * @param name The getCommand line parameter to retrieve
	 * @return const command_value& If the getCommand line parameter does not 
	 * exist, an empty variant is returned.
	 */
	const virtual command_value& get_parameter(const std::string& name) const;

	/**
	 * @brief getCommand interaction
	 */
...
/**
 * @brief User has issued a slash getCommand
 */
...
/**
 * @brief Discord requests that we fill a list of auto completion choices for a getCommand option
 */
